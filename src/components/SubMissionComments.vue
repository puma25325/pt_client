    ...
    <Card>
      ...
      <CardContent>
        ...
        <div v-else-if="comments.length === 0" class="text-center py-8 text-gray-500">
          ...
          <p>Aucun commentaire pour cette sous-mission</p>
        ...
        <div v-else class="space-y-4">
          ...
          >
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div class="flex items-center space-x-2 text-xs text-gray-500">
                    <Clock class="w-3 h-3" />
                    <span>{{ formatDate(comment.dateEnvoi || comment.createdAt) }}</span>
            ...
            <!-- Comment Metadata -->
            <!-- updatedAt field has been removed from server -->
          </div>
        </div>
      </CardContent>
    </Card>
...
import { GET_SUB_MISSION_COMMENTS_QUERY } from '@/graphql/queries/sub-mission-comments'
import { ADD_SUB_MISSION_COMMENT_MUTATION } from '@/graphql/mutations/sub-mission-comments'
...
// GraphQL
const { refetch: refetchComments } = useQuery(GET_SUB_MISSION_COMMENTS_QUERY, { subMissionId: props.subMissionId })
const { mutate: addSubMissionComment } = useMutation(ADD_SUB_MISSION_COMMENT_MUTATION)

// Computed
const sortedComments = computed(() => {
  return [...comments.value].sort((a, b) => 
    new Date(a.dateEnvoi || a.createdAt).getTime() - new Date(b.dateEnvoi || b.createdAt).getTime()
  )
})
...
const loadComments = async () => {
  ...
  try {
    ...
    if (result?.data?.getSubMissionComments) {
      // Create a new array to ensure it's mutable
      comments.value = [...result.data.getSubMissionComments]
    }
  } catch (error) {
    console.error('Error loading comments:', error)
    handleGraphQLError(error, 'Load Comments', { showToast: true })
  } finally {
    loading.value = false
  }
}
...
const sendComment = async () => {
  ...
  try {
    ...
    if (result?.data?.addSubMissionComment) {
      const newCommentData = result.data.addSubMissionComment
      // Create a new array to avoid modifying read-only GraphQL results
      comments.value = [...comments.value, newCommentData]
