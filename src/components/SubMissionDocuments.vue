    ...
    <Card class="shadow-sm border-0 bg-white">
      <CardHeader class="pb-4">
        <CardTitle class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <span class="text-lg font-semibold">Documents de la sous-mission ({{ documents.length }})</span>
          </div>
          <Button 
            v-if="canUpload" 
            variant="outline" 
      ...
      <CardContent class="pt-0">
        ...
        <div v-else-if="documents.length === 0" class="text-center py-8 text-gray-500">
          ...
          <p>Aucun document attaché à cette sous-mission</p>
...
import { GET_SUB_MISSION_DOCUMENTS_QUERY } from '@/graphql/queries/sub-mission-documents'
import { GET_DOCUMENT_DOWNLOAD_URL_QUERY } from '@/graphql/queries/mission-documents'
...
import SubMissionDocumentUpload from './SubMissionDocumentUpload.vue'
...
// GraphQL
const { refetch: refetchDocuments } = useQuery(GET_SUB_MISSION_DOCUMENTS_QUERY, { subMissionId: props.subMissionId })
...
const loadDocuments = async () => {
  ...
  try {
    ...
    if (result?.data?.getSubMissionDocuments) {
      // Create a new array to ensure it's mutable
      documents.value = [...result.data.getSubMissionDocuments]
    }
  } catch (error) {
...
const deleteDocument = async () => {
  ...
  deleting.value = true
  try {
    await deleteDocumentMutation({ documentId: documentToDelete.value.id })
